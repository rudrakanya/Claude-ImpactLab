  ...
  root.innerHTML = `
    ...
    <section class="glass card" id="onepager">
      ...
      ${tids.map((tid) => { const ts = state.tracks[tid]; const tr = TRACKS.tracks.find((x) => x.id === tid); const s = subjectById.get(ts.subjectId); const prog = computeProgress(ts); const nx = nextStepIndex(ts);
        ...
        <div class="small muted">${esc(tt({ en: "Placement", hi: "प्लेसमेंट" }))}: ${esc(t("selfSaid"))} ${esc(tt(TRACKS.tierLabels[ts.selfTier]))}, ${esc(tt({ en: "quiz", hi: "क्विज़" }))} ${ts.quiz ? `${ts.quiz.correct}/${ts.quiz.total}` : "–"} → ${esc(tt(TRACKS.tierLabels[ts.tier]))}${ts.path.templateId ? ` · ${esc(t("useTemplate"))}` : ""}</div>
